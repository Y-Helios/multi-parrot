...
# 检查server_single_diff_size.py和client.py是否存在
if [ ! -f "server_single_diff_size.py" ]; then
    echo "错误: 未找到server_single_diff_size.py文件"
    exit 1
fi
...
# 清理可能存在的旧进程
echo "清理可能存在的旧进程..."
pkill -f "python.*server_single_diff_size.py" 2>/dev/null || true
...
CUDA_LAUNCH_BLOCKING=1 CUDA_VISIBLE_DEVICES=0 python engine_diff_size.py --start-api-server --api-host "$ENGINE1_HOST" --port "$ENGINE1_PORT" \
    --torch-dtype float16 \
    --attn-implementation flash_attention_2 \
    --temperature 0.2 \
    --max-new-tokens 1024 \
    --image-size $IMAGE_SIZE > engine.log 2>&1 &
ENGINE_PID=$!
...
for i in {1..60}; do
    sleep 10
    echo "等待Engine中... ($i/60) - 已等待 $((i*10)) 秒"
...
# 启动服务器（后台运行）
echo "正在启动服务器..."
echo "执行命令: python server_single_diff_size.py --host $SERVER_HOST --port $SERVER_PORT --image-size $IMAGE_SIZE"

# 将服务器的输出重定向到文件以便调试
python server_single_diff_size.py --host "$SERVER_HOST" --port "$SERVER_PORT" --image-size $IMAGE_SIZE > server.log 2>&1 &
SERVER_PID=$!
