...
def round_record(round_num: int, engine_num: str, text_num: int, image_num: int, server_send_time: float, payload_mem: float,
              ...
              server_receive_time: float, engine_send_packet_mem: float,
              load_start_time: float, load_end_time: float, 
              preprocess_start_time: float, preprocess_end_time: float) -> dict:
...
def call_engine_generate_text_batch(input_texts: List[str], image_paths: List[List[str]], round_num: int = 0) -> tuple[List[str], dict]:
    ...
    try:
        ...
        # print(f"result.get('load_start_time'): {result.get('load_start_time')}")
        # print(f"result.get('load_end_time'): {result.get('load_end_time')}")
        # print(f"result.get('preprocess_start_time'): {result.get('preprocess_start_time')}")
        # print(f"result.get('preprocess_end_time'): {result.get('preprocess_end_time')}")
        ...
        record_dict = round_record(
            ...
            load_start_time=result.get('load_start_time', 0.0),
            load_end_time=result.get('load_end_time', 0.0),
            preprocess_start_time=result.get('preprocess_start_time', 0.0),
            preprocess_end_time=result.get('preprocess_end_time', 0.0)
        )
...
def process_batch_data(batch_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if input_texts:
        ...
        sample_results = [""] * batch_size      # Save final results for each sample
        ...
        # Process samples that are not yet finished at the last round
...
def process_batch(request: BatchData):
    try:
        ...
        output_dir = f"/scratch/jsb5jd/LLaVA-NeXT/demo_a_sys/output_dir3_cpu/bs{batch_size}_size64_single_engine"
