...
# 检查命令行参数
if [ $# -eq 0 ]; then
    echo "用法: $0 <image_size>"
    echo "示例: $0 320"
    echo "默认image_size: 320"
    IMAGE_SIZE="320"
elif [ $# -eq 1 ]; then
    # 检查参数是否为数字
    if [[ $1 =~ ^[0-9]+$ ]]; then
        IMAGE_SIZE="$1"
        echo "使用指定的image_size: $IMAGE_SIZE"
    else
        echo "错误: image_size必须是数字"
        echo "用法: $0 <image_size>"
        echo "示例: $0 320"
        exit 1
    fi
...
# 检查server_diff_size.py和client.py是否存在
if [ ! -f "server_diff_size.py" ]; then
    echo "错误: 未找到server_diff_size.py文件"
    exit 1
fi
...
if [ ! -f "engine_diff_size.py" ]; then
    echo "错误: 未找到engine_diff_size.py文件"
    exit 1
fi
...
# 清理可能存在的旧进程
echo "清理可能存在的旧进程..."
pkill -f "python.*server_diff_size.py" 2>/dev/null || true
pkill -f "python.*engine_diff_size.py" 2>/dev/null || true
# 清理可能占用的端口
...
echo "执行命令: CUDA_VISIBLE_DEVICES=0 python engine_diff_size.py --start-api-server --api-host $ENGINE1_HOST --port $ENGINE1_PORT --image-size $IMAGE_SIZE"

CUDA_VISIBLE_DEVICES=0 python engine_diff_size.py --start-api-server --api-host "$ENGINE1_HOST" --port "$ENGINE1_PORT" \
    --torch-dtype float16 \
    --attn-implementation flash_attention_2 \
    --temperature 0.2 \
    --max-new-tokens 1024 \
    --image-size $IMAGE_SIZE > engine1.log 2>&1 &
ENGINE1_PID=$!
...
echo "执行命令: CUDA_VISIBLE_DEVICES=1 python engine_diff_size.py --start-api-server --api-host $ENGINE2_HOST --port $ENGINE2_PORT --image-size $IMAGE_SIZE"

CUDA_VISIBLE_DEVICES=1 python engine_diff_size.py --start-api-server --api-host "$ENGINE2_HOST" --port "$ENGINE2_PORT" \
    --torch-dtype float16 \
    --attn-implementation flash_attention_2 \
    --temperature 0.2 \
    --max-new-tokens 1024 \
    --image-size $IMAGE_SIZE > engine2.log 2>&1 &
ENGINE2_PID=$!
...
# 启动服务器（后台运行）
echo "正在启动服务器..."
echo "执行命令: python server_diff_size.py --host $SERVER_HOST --port $SERVER_PORT --image-size $IMAGE_SIZE"

# 将服务器的输出重定向到文件以便调试
python server_diff_size.py --host "$SERVER_HOST" --port "$SERVER_PORT" --image-size $IMAGE_SIZE > server.log 2>&1 &
SERVER_PID=$!
...
if [ -f "engine1.log" ]; then
    mv engine1.log "logs/engine1_${TIMESTAMP}.log"
    echo "Engine1日志已保存到: logs/engine1_${TIMESTAMP}.log"
fi
if [ -f "engine2.log" ]; then
    mv engine2.log "logs/engine2_${TIMESTAMP}.log"
    echo "Engine2日志已保存到: logs/engine2_${TIMESTAMP}.log"
fi
