...
parser.add_argument("--api-port", type=int, default=9001, help="API server port")
parser.add_argument("--api-host", type=str, default="localhost", help="API server host")
parser.add_argument("--start-api-server", action="store_true", help="Start API server mode")
parser.add_argument("--port", type=int, default=9001, help="API server port (alias for --api-port)")
...
def preprocess_qwen_infer(
    ...
    # If the first speaker is not user, skip it (not compliant)
    source = sources
    ...
    # The last round is user query, followed by assistant to wait for model generation
    last_user = source[-1]
    if roles[last_user["from"]] != "<|im_start|>user":
        raise ValueError("the last round must be user query, for generating")
...
def load_and_process_images(all_image_paths, vision_tower_device):
    ...
    images = [Image.open(args.image_folder + p).convert("RGB") for sublist in all_image_paths for p in sublist]
...
def generate_text_batch(input_texts, image_paths, round_num=0):
    """
    batch
    """        
    ...
    print(f"global_image_embeds:")
    for item in global_image_embeds:
        print(f"round: {item['round']}")
        print(f"image_embeds: {item['image_embeds'].shape}")
    ...
    if all_input_texts is not None:
        ...
        if any(all_has_images):
            ...
            if all_image_paths is not None and any(len(x) > 0 for x in all_image_paths):
                # Image paths passed
                print(f"obtain image paths for round {round_num}")
                ...
                image_tensor_mem = image_tensors.element_size() * image_tensors.nelement() / (1024 * 1024)  # MB
                image_embed_mem = image_embeds.element_size() * image_embeds.nelement() / (1024 * 1024)  # MB
                
            else:
                # No image path passed, possibly pre-coded
                print(f"loading pre-encoded embeds for round {round_num}")
                found = next((item for item in global_image_embeds if item["round"] == round_num), None)
                if found is not None:
                    device = next(model.parameters()).device
                    image_embeds = found["image_embeds"].to(device=device)
                    image_tensors = None
                else:
                    print(f"did not pre-encode images for round {round_num}")
                    return None
            ...
            print(f"no image in text for round {round_num}")
            # text has no <image>, pure text
            prefill_start = time.time()
            with torch.inference_mode():
                # Pure text input
                with torch.cuda.amp.autocast(dtype=torch.bfloat16):
        ...
        if all_image_paths is not None and any(len(x) > 0 for x in all_image_paths):
            # No text, but image paths, meaning pre-encode images are needed
            print(f"no text but image paths for round {round_num}")
            vision_tower_device = model.get_model().get_vision_tower().device
            image_tensor_start = time.time()
            image_tensors = load_and_process_images(all_image_paths, vision_tower_device)
            image_tensors = image_tensors.to(dtype=torch.float16)
            ...
            image_embeds = image_embeds.to(device=device)
            image_tensors = image_tensors.to(device=device)
            # Cache current round embedding
            global_image_embeds.append({"round": round_num, "image_embeds": image_embeds})
            return {"status": "embedding_cached"}
        else:
            # No text, no image path, meaning no image
            print(f"Error: no text neither image path for round {round_num}")
            return None
    ...
    # for step in range(20):
        with torch.inference_mode():
            ...
            # Ensure next_token is on the correct device
            ...
            if all(finished):
                print(f"All samples finished generating for round {round_num}")
                break
            # if finished.all():
            #     break
    ...
    return all_generated_texts, image_tensor_start, image_tensor_end, image_embed_start, image_embed_end, prefill_start, prefill_end, decode_start, decode_end, image_tensor_mem, image_embed_mem
...
class GenerateResponse(BaseModel):
    outputs: List[str]
    status: str = "success"
    engine_receive_time: float = 0.0
    server_send_packet_mem: float = 0.0
    image_tensor_start: float = 0.0
    image_tensor_end: float = 0.0
    image_encode_start: float = 0.0
    image_encode_end: float = 0.0
    prefill_start: float = 0.0
    prefill_end: float = 0.0
    decode_start: float = 0.0
    decode_end: float = 0.0
    engine_send_time: float = 0.0
    tensor_mem: float = 0.0
    embed_mem: float = 0.0
    output_mem: float = 0.0

@app.post("/generate_text_batch", response_model=GenerateResponse)
async def api_generate_text(request: Request, body: GenerateRequest):
    """HTTP API interface to call generate_text_batch function"""
    try:
        engine_receive_time = time.time()
        # Calculate the size of the entire request packet
        headers_size = sum(len(key.encode('utf-8')) + len(str(value).encode('utf-8')) for key, value in request.headers.items())
        content_size = len(await request.body())
        method_size = len(request.method.encode('utf-8'))
        url_size = len(str(request.url).encode('utf-8'))
        # Convert to MB
        server_send_packet_mem = (headers_size + content_size + method_size + url_size) / (1024 * 1024)  # MB

        # print(f"Engine API received request:")
        # print(f"  input_texts: {body.input_texts}")
        # print(f"  image_paths: {body.image_paths}")
        
        # Pass round_num, ensure it's int
        round_num = body.round_num if body.round_num is not None else 0
        outputs_tuple = generate_text_batch(body.input_texts, body.image_paths, round_num)
        if outputs_tuple is None:
            raise Exception("generate_text_batch returned None")
        if isinstance(outputs_tuple, dict) and outputs_tuple.get("status") == "embedding_cached":
            # embedding pre-encoding request, return empty outputs directly
            return GenerateResponse(
                outputs=[],
                engine_receive_time=engine_receive_time,
                server_send_packet_mem=server_send_packet_mem,
                image_tensor_start=0.0,
                image_tensor_end=0.0,
                image_encode_start=0.0,
                image_encode_end=0.0,
                prefill_start=0.0,
                prefill_end=0.0,
                decode_start=0.0,
                decode_end=0.0,
                engine_send_time=time.time(),
                tensor_mem=0.0,
                embed_mem=0.0,
                output_mem=0.0
            )
        outputs, image_tensor_start, image_tensor_end, image_embed_start, image_embed_end, prefill_start, prefill_end, decode_start, decode_end, image_tensor_mem, image_embed_mem = outputs_tuple
        
        # print(f"Engine API returned results: {outputs}")
        ...
        return GenerateResponse(
            ...
            image_encode_start=image_embed_start,
            image_encode_end=image_embed_end,
            prefill_start=prefill_start,
            prefill_end=prefill_end,
            decode_start=decode_start,
            decode_end=decode_end,
            engine_send_time=engine_send_time,
            tensor_mem=image_tensor_mem,
            embed_mem=image_embed_mem,
            output_mem=sys.getsizeof(str(outputs)) / (1024 * 1024)  # Convert to MB
        )
...
def main():
    ...
    for batch_start in range(0, num_samples, batch_size):
        ...
        for r in range(1):
            ...
            # Optimized: rounds with fewer inputs still input text, as they are not optimized
            # print(f"new_cur_input_text: {new_cur_input_text}")
            # print(f"new_image_paths: {new_image_paths}")

            print(f"BATCH: {batch_num}, Round: {r+1}")
            # output1 = generate_text_batch(None, new_image_paths)
            # print(f"output1: {output1.shape}")
            # output = generate_text_batch(new_cur_input_text, None)
            output = generate_text_batch(new_cur_input_text, new_image_paths, int(r))
...
def start_api_server():
    ...
    # Use --port parameter, if not set, use --api-port
    port = args.port if hasattr(args, 'port') and args.port != 9001 else args.api_port
    print(f"Starting Engine API server at {args.api_host}:{port}")
    uvicorn.run(app, host=args.api_host, port=port)
