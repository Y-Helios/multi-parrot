...
def get_next_engine_url() -> tuple[str, str]:
    ...
    Returns:
        ...
        - engine_num: Number of the engine ("engine1" or "engine2")
    """
...
def process_batch_data(batch_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if input_texts:
        ...
        sample_finished = [False] * batch_size  # Mark which samples are finished
        ...
        for r in range(max_round):
            ...
            if active_indices:
                ...
                # No active samples left, all finished
                # print(f"Round {r+1} - All samples are finished")
                break
...
def process_batch(request: BatchData):
    try:
        ...
        output_dir = f"/scratch/jsb5jd/LLaVA-NeXT/demo_a_sys/output_dir4_test/image_size{args.image_size}_bs4_single_engine_cpu"
