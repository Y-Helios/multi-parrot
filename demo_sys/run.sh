...
CUDA_VISIBLE_DEVICES=0 python engine.py --start-api-server --api-host "$ENGINE1_HOST" --port "$ENGINE1_PORT" \
    --torch-dtype float16 \
...
echo "执行命令: CUDA_VISIBLE_DEVICES=1 python engine.py --start-api-server --api-host $ENGINE2_HOST --port $ENGINE2_PORT"

CUDA_VISIBLE_DEVICES=1 python engine.py --start-api-server --api-host "$ENGINE2_HOST" --port "$ENGINE2_PORT" \
    --torch-dtype float16 \
    --attn-implementation flash_attention_2 \
    --temperature 0.2 \
    --max-new-tokens 1024 \
...
# 启动服务器（后台运行）
echo "正在启动服务器..."
echo "执行命令: python server.py --host $SERVER_HOST --port $SERVER_PORT"

# 将服务器的输出重定向到文件以便调试
python server.py --host "$SERVER_HOST" --port "$SERVER_PORT" > server.log 2>&1 &
SERVER_PID=$!
