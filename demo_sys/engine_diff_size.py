...
parser.add_argument("--image-size", type=int, default=640, help="Image size for processing (width,height).")
...
parser.add_argument("--max-samples", type=int, default=4, help="Max number of samples to process (0=all).")
...
# print(f"model.model.layers[0].self_attn.__class__: {model.model.layers[0].self_attn.__class__}")
...
def preprocess_qwen_infer(
    ...
    # Special token IDs
    im_start, im_end, img_token = tokenizer.additional_special_tokens_ids
    nl_tokens = tokenizer("\n").input_ids
    ...
    # The last round is user query, followed by assistant to generate
    last_user = source[-1]
...
def load_and_process_images(all_image_paths, vision_tower_device):
    ...
    # image_tensor = image_processor.preprocess(images=images, return_tensors='pt')['pixel_values']
...
def generate_text_batch(input_texts, image_paths):
    ...
    if all_input_texts is not None:
        ...
        if any(all_has_images):
            ...
            if all_image_paths is not None and any(len(x) > 0 for x in all_image_paths):
                ...
                image_tensors, load_start_time, load_end_time, preprocess_start_time, preprocess_end_time = load_and_process_images(all_image_paths, vision_tower_device)
                ...
                # print(f"global_image_embeds: {global_image_embeds}")
    ...
    return all_generated_texts, image_tensor_start, image_tensor_end, image_embed_start, image_embed_end, prefill_start, prefill_end, decode_start, decode_end, image_tensor_mem, image_embed_mem, load_start_time, load_end_time, preprocess_start_time, preprocess_end_time
...
class GenerateResponse(BaseModel):
    ...
    preprocess_start_time: float = 0.0
    preprocess_end_time: float = 0.0
...
async def api_generate_text(request: Request, body: GenerateRequest):
    """HTTP API interface calls generate_text_batch function"""
    try:
        ...
        outputs, image_tensor_start, image_tensor_end, image_embed_start, image_embed_end, prefill_start, prefill_end, decode_start, decode_end, image_tensor_mem, image_embed_mem, load_start_time, load_end_time, preprocess_start_time, preprocess_end_time = generate_text_batch(body.input_texts, body.image_paths)
...
def start_api_server():
    ...
    # Use --port parameter, if not, use --api-port
