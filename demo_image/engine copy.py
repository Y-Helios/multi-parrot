...
parser.add_argument("--max-samples", type=int, default=128, help="Max number of samples to process (0=all).")
...
def generate_text_batch(input_texts, image_paths):
    ...
    if all_input_texts is not None:
        ...
        if all_image_paths is not None and any(len(x) > 0 for x in all_image_paths):
            # No text, but image paths, meaning pre-encoding images is needed
