...
parser.add_argument("--run-time", type=int, default=700, help="Run time in seconds (default: 600 = 10 minutes)")
...
def load_and_process_images(all_image_paths):
    ...
    image_tensor = image_processor.preprocess(images=images, return_tensors='pt', size=args.image_size)['pixel_values']
    return image_tensor
...
def record_batch_data(batch_count, batch_size, image_tensors, image_embeds, 
    ...
    batch_data = {
        ...
        "average_time_per_image": round(embed_time / batch_size, 6),
...
def main():
    ...
    if end_time > 0:
        while time.time() < end_time:
            ...
            image_tensors = load_and_process_images(batches)
            image_tensors = image_tensors.to(dtype=torch.float16)
            image_tensor_end = time.time()

            # Automatically detect which GPU vision_tower is on and move image tensor to the corresponding device
            ...
            record_batch_data(batch_count, batch_size, image_tensors, image_embeds, image_embed_start, image_embed_end, image_tensor_start, image_tensor_end)
    ...
    # Build save path
    file_path = os.path.join(log_dir, f"img_encode_bs{args.batch_size}_size{args.image_size}_{time_str}.json")
