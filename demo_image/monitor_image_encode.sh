...
LOG_DIR="image_encode_gpu_info"
RUN_LOG_DIR="image_encode_run_log"
mkdir -p "$LOG_DIR"
mkdir -p "$RUN_LOG_DIR"
...
CUDA_VISIBLE_DEVICES=0 python batch_image_loader.py --image-size=64 --batch-size=${bs} > "$RUN_LOG_DIR/engine_bs${bs}_${start_time}.log" 2>&1 &
ENGINE_PID=$!
