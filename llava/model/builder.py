    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                # 获取已有的 special tokens
                old_special_tokens = tokenizer.special_tokens_map.get("additional_special_tokens", [])

                # 如果 '<image>' 不在里面就加进去
                if "<image>" not in old_special_tokens:
                    new_special_tokens = old_special_tokens + ["<image>"]
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        print(f"refer to qwen example (overwrite_config is not None):")
                        ...
                        # #     trust_remote_code=True,
                        # #     ).eval()
                        # model = LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=config, **kwargs)

                        # model.genneration_config = GenerationConfig.from_pretrained(model_path, pad_token_id=tokenizer.pad_token_id)
